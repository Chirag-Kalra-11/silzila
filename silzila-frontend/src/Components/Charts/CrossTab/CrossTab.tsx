...
export const getColumnList = (index: number, list: any[]) => {
  ...
  for (let i = 0; i < chartColumns.length; i++) {
    ...
    if (
      !_columnList.includes(col) //|| col === "Total"
    ) {
...
export const getFilteredChartPropDataByCompareObject = (
  ...
  const finder = (item: any) => {
    ...
    if (keys && keys.length > 0) {
      keys.forEach((key) => {
        // console.log(key);
        if (isEqual && key !== "total") {
          isEqual = item[key]?.toString() === compareObj[key]?.toString();
        } //||
        // compareObj[key]?.toString() === "Total";
      });
...
export const getFilteredChartPropDataByCompareObject2 = (
  propData: any,
  compareObj: any,
  // data?: any
  int: number
  ...
  const finder = (item: any) => {
    ...
    // console.log(keys);
    let index = 0;
    if (keys && keys.length > 0 && index < int) {
      keys.forEach((key) => {
        ...
        // console.log(key);
        if (
          (isEqual && key !== "total" && item[key] !== undefined) ||
          (item[key] === undefined && compareObj[key])
        ) {
          isEqual = item[key]?.toString() === compareObj[key]?.toString();
          // console.log(item[key]?.toString(), compareObj[key]?.toString());
        } //||
        // compareObj[key]?.toString() === "Total";
        index++;
      });
...
export const getDistinctList = (
  ...
  const finder = (item: any, modifiedCompareObj: any) => {
    ...
    keys.forEach((key) => {
      if (isEqual && key !== "total")
        isEqual = item[key] === modifiedCompareObj[key];
    });
  ...
  list.forEach((item) => {
    ...
    if (
      resultList.length === 0 ||
      !resultList.find((item) => finder(item, modifiedCompareObj))
    ) {
...
export const getUserClickedClassNameForColor = (
  ...
  let _filteredData = getFilteredChartPropDataByCompareObject(
    chartPropData,
    userCellCompareJSON
  );
...
*/
export const addDusbinValuesMeasuresInChartData = (
  dustbinValues: any,
  list: any
) => {
  let newRowsArray = [];
